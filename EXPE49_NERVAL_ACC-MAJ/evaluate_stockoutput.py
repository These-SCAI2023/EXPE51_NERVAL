...
def compute_scores(
    annot_tags_count: dict, predict_tags_count: dict, matches: dict, annotation: str
) -> dict:
    ...
    for tag in sorted(tags)[::-1]:
        ...
        # with open(f"{annotation}_resultats.txt", "w", encoding="utf-8") as fichier:
        with open(f"{annotation}_resultats.txt", "w") as fichier:
            # fichier.write(str(results))
            fichier.write(json.dumps(scores, indent=2))

    return scores


# def print_results(scores: dict, annotation: str):
...
def run(annotation: str, prediction: str, threshold: int, verbose: bool) -> dict:
    ...
    # Compute scores
    scores = compute_scores(annot["entity_count"], predict["entity_count"], matches, annotation)
    ...
    with open(f"{annotation}_aligned.txt", "w", encoding="utf-8") as f:
        f.write(string_bio)
    ...
    if verbose:
        ...
        # print_results(scores, annotation)
    else:
